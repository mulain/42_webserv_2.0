...
{
	if (buffer->find("\r\n") == std::string::npos)
		throw ErrorCode(400, prependClassName(__FUNCTION__));
...
void Request::parseRequestHeaders()
{
	if (buffer->find("\r\n\r\n") == std::string::npos)
	{
		if (buffer->size() >= MAX_REQHEADSIZE)
			throw ErrorCode(431, prependClassName(__FUNCTION__));
		else
			throw ErrorCode(400, prependClassName(__FUNCTION__));
	}
...
void Request::requestError()
{
	if (_httpProtocol != HTTPVERSION)
		throw ErrorCode(505, prependClassName(__FUNCTION__));

	if (_method != GET && _method != POST && _method != DELETE)
		throw ErrorCode(501, prependClassName(__FUNCTION__));

	if (_contentLength > _activeConfig->getClientMaxBody())
		throw ErrorCode(413, prependClassName(__FUNCTION__));
	...
	if (it == _activeConfig->getLocations()->end())
	{
		if ((_method == GET || _method == DELETE) && !resourceExists(prependRoot(_URL)))
			throw ErrorCode(404, prependClassName(__FUNCTION__)); // can't check before in case of http redir

		throw ErrorCode(403, prependClassName(__FUNCTION__)); // should always 404 on a production system to not leak file structure
	}
	...
	if ((_method == GET && !_locationInfo.get)
		...
		throw ErrorCode(405, prependClassName(__FUNCTION__));
}
...
{	
	...
	if (_method == POST && !_locationInfo.upload_dir.empty()) // upload_redir supercedes http_redir
	{
		...
		if (_activeConfig->getLocations()->find(_updatedDirectory) == _activeConfig->getLocations()->end())
			throw ErrorCode(403, prependClassName(__FUNCTION__)); // upload_redir also has to be set in the config file
