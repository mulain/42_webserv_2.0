...
// status page sending constructor
...
Response::Response(dynCont genContent, const Request& request):
	_request(request)
{
	sendSelector = &Response::sendInternalBuffer;
	switch (genContent)
...
std::string Response::buildResponseHead(int code)
{
	std::stringstream	ss;
	
	ss	<< HTTPVERSION << ' ' << code << ' ' << getHttpMsg(code) << "\r\n"
